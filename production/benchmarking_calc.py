...
#Extract raw_data
os.getcwd()
raw_data_dir = os.path.join(os.getcwd(), paths.data_extracted_dir)
os.chdir(raw_data_dir)
print(os.listdir()[0])
with open(os.listdir()[0], 'rb') as f:
    raw_data = joblib.load(f)

print(f"Selected extract has {len(raw_data.index)} records")

#Begin Benchmarking-specific preprocessing
#Assumption - data in extract is only for last 12 months
#Extract relevant features for building similarity metrics
...
#for production
def create_similarity_score_matrix(raw_data):
    params = get_similarity_score_params(raw_data)
...
def add_standard_methods(arg_df=raw_data):
    '''
...
The 9 KPI Metrics are as follows:
    1. Average total spend (1 column)
    2. Average total discounts (1 column)
    3. Average spend per shipping method (7 columns)
    4. Average spend per month (7 columns)
    5. Average discounts per shipping method (7 columns)
    6. Proportion of shipping methods used (7 columns)
    7. Total volume shipped per shipping method (7 columns)
    8. Proportion of each carrier (FedEx, UPS) used (2 columns)
    9. Average discounts per zone (However many columns for zones used)
'''
    print('Compiling Customer Metrics')
    #Step 0.0: Create net discount column
    arg_df['net_discount_amount'] = np.add(abs(np.array(raw_data['freight_discount_amount'])),\
                            np.array(raw_data['misc_discount_amount'])  )
    ...
    # 5.4 Average total spend per method, per pound, per shipper
...
a = create_customer_KPI_database(raw_data)
